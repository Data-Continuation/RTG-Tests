...
TMP = ROOT / "build" / "test_rtg_returned_round_trip_audit_record"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["audit"].read_text()); require(data["round_trip_status"]=="round_trip_complete","round trip not complete"); require(data["audit_summary"]["claim_boundary_confirmed"] is True,"audit claim mismatch")
    print("RTG returned round-trip audit record tests passed.")
