#!/usr/bin/env python3
"""Provisional RTG receipt-chain continuity tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "receipt-chain-continuity.valid.json"

def classify(c):
    if not c["prior_state_present"]: return "missing_prior"
    if not c["receipt_link_valid"]: return "receipt_break"
    if not c["result_state_present"]: return "missing_result"
    if not c["export_receipt_present"]: return "export_missing"
    return "continuous"
...
def main() -> None:
    ...
    print("RTG receipt-chain continuity tests passed.")
