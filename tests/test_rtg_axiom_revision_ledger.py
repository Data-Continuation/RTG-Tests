...
TMP=ROOT/"build"/"axiom-revision-ledger-test"
def load(name,path):
...
def main():
    ax,_,_,_=seed_inputs(); out=TMP/"ledger.json"; load("led",ROOT/"scripts"/"ledger_rtg_axiom_revisions.py").build_record({"axioms":ax},out)
    p=json.loads(out.read_text()); require(p["summary"]["ledger_entry_count"]==2,"ledger count mismatch"); require(p["summary"]["ledger_posture"]=="revision_tracking_active","posture mismatch")
    print("RTG axiom revision ledger tests passed.")
