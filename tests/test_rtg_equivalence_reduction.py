...
TMP = ROOT / "build" / "equivalence-reduction-test"
...
def main():
    ...
    ax = TMP/"axioms.json"; op = TMP/"operators.json"; out = TMP/"equivalence.json"
    write_json(ax, axioms); write_json(op, operators)
    mod = load("equivalence", ROOT/"scripts"/"reduce_rtg_equivalence_classes.py")
    mod.reduce(ax, op, out)
    payload = json.loads(out.read_text())
    require(payload["summary"]["equivalence_class_count"] >= 1, "expected equivalence classes")
    require(payload["summary"]["candidate_operator_count"] == 2, "operator count mismatch")
    print("RTG equivalence reduction tests passed.")
