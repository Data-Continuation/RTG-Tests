...
TMP = ROOT / "build" / "test_rtg_actual_solver_upload_bundle_manifest"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["upload_manifest"].read_text()); require(data["manifest_status"]=="ready_for_upload","manifest not ready"); require(len(data["upload_files"])==3,"upload file count mismatch")
    print("RTG actual solver upload bundle manifest tests passed.")
