...
TMP=ROOT/"build"/"external-theorem-prover-adapter-test"
def load(name,path):
...
def main():
    _,_,_,obl=seed_inputs(); pc=TMP/"pc.json"; thm=TMP/"thm.json"; out=TMP/"prover.json"
    load("pc",ROOT/"scripts"/"generate_rtg_proof_candidates.py").build_record({"proof_obligations":obl},pc); load("thm",ROOT/"scripts"/"register_rtg_theorem_candidates.py").build_record({"proof_candidates":pc},thm); load("prover",ROOT/"scripts"/"adapt_rtg_external_theorem_prover.py").build_record({"theorem_candidates":thm},out)
    p=json.loads(out.read_text()); require(p["summary"]["prover_request_count"]==3,"prover count mismatch"); require(p["summary"]["adapter_posture"]=="ready_for_external_prover","posture mismatch")
    print("RTG external theorem-prover adapter tests passed.")
