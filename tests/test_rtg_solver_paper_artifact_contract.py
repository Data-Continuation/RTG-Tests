...
TMP = ROOT / "build" / "solver-paper-artifact-contract-test"
...
def main():
    p = pipeline()
    data = json.loads(p["contract"].read_text())
    require(data["artifact_contract_valid"] is True, "contract invalid")
    require(data["artifact_type"] == "solver_paper_or_report", "artifact type mismatch")
    print("RTG solver paper artifact contract tests passed.")
