...
TMP = ROOT / "build" / "tv-tvc-authority-request-test"
...
def main():
    ...
    out = TMP / "authority_request.json"
    mod = load("request", ROOT / "scripts" / "request_rtg_tv_tvc_math_solver_authority.py")
    mod.build_authority_request(manifest, out)
    payload = json.loads(out.read_text())
    require(payload["authority_route"]["tv_repo"] == "StegVerse-Labs/TV", "TV repo mismatch")
    require(payload["authority_route"]["tvc_repo"] == "StegVerse-Labs/TVC", "TVC repo mismatch")
    require(payload["authority_route"]["credential_boundary"] == "tv_tvc_brokered", "credential boundary mismatch")
    require(payload["target_workflow"]["owner"] == "GCAT-BCAT-Engine", "target owner mismatch")
    require(payload["case_count"] == 1, "case count mismatch")
    print("RTG TV/TVC authority request tests passed.")
