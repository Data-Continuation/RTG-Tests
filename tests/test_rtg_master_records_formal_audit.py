...
TMP=ROOT/"build"/"master-records-formal-audit-test"
def load(name,path):
...
def main():
    _,_,_,obl=seed_inputs(); pc=TMP/"pc.json"; thm=TMP/"thm.json"; mc=TMP/"mc.json"; pr=TMP/"pr.json"; exp=TMP/"exp.json"; out=TMP/"audit.json"
    load("pc",ROOT/"scripts"/"generate_rtg_proof_candidates.py").build_record({"proof_obligations":obl},pc); load("thm",ROOT/"scripts"/"register_rtg_theorem_candidates.py").build_record({"proof_candidates":pc},thm); load("mc",ROOT/"scripts"/"adapt_rtg_model_check.py").build_record({"theorem_candidates":thm},mc); load("pr",ROOT/"scripts"/"adapt_rtg_external_theorem_prover.py").build_record({"theorem_candidates":thm},pr); load("ex",ROOT/"scripts"/"export_rtg_formal_evidence.py").build_record({"proof_candidates":pc,"theorem_candidates":thm,"model_check_adapter":mc,"external_theorem_prover_adapter":pr},exp); load("audit",ROOT/"scripts"/"audit_rtg_master_records_formal.py").build_record({"formal_evidence_export":exp},out)
    p=json.loads(out.read_text()); require(p["summary"]["audit_item_count"]==4,"audit count mismatch"); require(p["summary"]["audit_posture"]=="master_records_candidate_ready","posture mismatch")
    print("RTG master-records formal audit tests passed.")
