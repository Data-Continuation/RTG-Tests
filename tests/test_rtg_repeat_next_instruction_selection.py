...
TMP = ROOT / "build" / "test_rtg_repeat_next_instruction_selection"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["selection"].read_text()); require(data["selected_instruction_class"]=="next_candidate_vector_expansion","selection mismatch")
    print("RTG repeat next instruction selection tests passed.")
