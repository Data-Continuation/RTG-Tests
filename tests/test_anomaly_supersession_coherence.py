#!/usr/bin/env python3
"""Provisional RTG anomaly supersession coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "anomaly-supersession-coherence.valid.json"

def classify(c):
    if c["new_version"] < c["prior_version"]: return "older_anomaly_attempt_blocked"
    if c["risk"] > 0.60: return "supersession_quarantine_anomaly"
    if c["anomaly_present"] and not c["anomaly_resolved"]: return "supersession_blocked_by_anomaly"
    if not c["anomaly_present"]: return "supersession_anomaly_provisional"
    return "supersession_with_resolved_anomaly"
...
def main() -> None:
    ...
    print("RTG anomaly supersession coherence tests passed.")
