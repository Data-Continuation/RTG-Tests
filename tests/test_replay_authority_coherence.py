#!/usr/bin/env python3
"""Provisional RTG replay authority coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "replay-authority-coherence.valid.json"

def classify(c):
    if c["receipt_conflict"] or c["known_invalid_signal"]:
        return "coherence_quarantine"
    if c["replay_valid"] and c["authority_satisfied"]:
        return "coherent_authorized_replay"
    if c["replay_valid"] and not c["authority_satisfied"]:
        return "replay_valid_authority_missing"
    if not c["replay_valid"] and c["authority_satisfied"]:
        return "authority_valid_replay_missing"
    return "both_missing"
...
def main() -> None:
    ...
    print("RTG replay authority coherence tests passed.")
