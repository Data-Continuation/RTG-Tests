#!/usr/bin/env python3
"""Provisional RTG unknown unknown review loop coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "unknown-unknown-review-loop-coherence.valid.json"

def classify(c):
    if c["resolved"]: return "review_loop_close"
    if c["risk"] > 0.60: return "review_loop_escalate"
    if c["new_evidence"]: return "review_loop_regrade"
    if c["observer_gap"] > 0.60: return "review_loop_widen"
    return "review_loop_preserve"
...
def main() -> None:
    ...
    print("RTG unknown unknown review loop coherence tests passed.")
