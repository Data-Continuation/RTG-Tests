...
TMP=ROOT/"build"/"proof-candidate-generation-test"
def load(name,path):
...
def main():
    _,_,_,obl=seed_inputs(); out=TMP/"proof_candidates.json"
    load("pc",ROOT/"scripts"/"generate_rtg_proof_candidates.py").build_record({"proof_obligations":obl},out)
    p=json.loads(out.read_text()); require(p["summary"]["proof_candidate_count"]==3,"proof candidate count mismatch"); require(p["summary"]["open_candidate_count"]==0,"open count mismatch")
    print("RTG proof-candidate generation tests passed.")
