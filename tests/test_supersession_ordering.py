#!/usr/bin/env python3
"""Provisional RTG supersession ordering tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "supersession-ordering.valid.json"

def classify(c):
    if c["new_version"] < c["prior_version"]: return "older_attempt_blocked"
    if c["new_version"] == c["prior_version"]: return "same_version_conflict"
    if not c["predecessor_present"]: return "missing_predecessor"
    if c["ordering_confidence"] < 0.60: return "supersession_provisional"
...
def main() -> None:
    ...
    print("RTG supersession ordering tests passed.")
