...
TMP = ROOT / "build" / "solver-publication-artifact-gate-test"
...
def main():
    p = pipeline()
    data = json.loads(p["gate"].read_text())
    require(data["publication_status"] == "publication_candidate_allowed_with_boundaries", "publication status mismatch")
    require("human or formal verification required" in data["required_language"], "required language missing")
    print("RTG solver publication artifact gate tests passed.")
