...
TMP = ROOT / "build" / "math-solver-upload-spec-contract-test"
...
def main():
    p = pipeline()
    data = json.loads(p["upload"].read_text())
    require(".github/workflows/validation_run_inline.yml" in data["workflow_targets"], "workflow target missing")
    require(data["target_paths"]["problem_spec"].endswith("problem_spec_rtg_instruction.yml"), "problem spec mismatch")
    require("publisher_document_or_solver_report.yml" in data["expected_artifacts"], "expected artifact missing")
    print("RTG math-solver upload spec contract tests passed.")
