#!/usr/bin/env python3
"""Provisional RTG false transition receipt quarantine coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "false-transition-receipt-quarantine-coherence.valid.json"

VALID_STATES = ['false_transition_quarantined', 'authority_failure', 'lineage_failure', 'receipt_failure', 'risk_quarantine']
...
def classify(case: dict[str, Any]) -> str:
    ...
    if not case["secondary_ok"]:
        return "receipt_failure"
    return "false_transition_quarantined"
...
def main() -> None:
    ...
    print("RTG false transition receipt quarantine coherence tests passed.")
