...
TMP = ROOT / "build" / "test_rtg_repeat_solver_loop_state_update"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["loop_state"].read_text()); require(data["state_status"]=="repeat_loop_state_updated","state status mismatch"); require(data["loop_iteration"]==1,"loop iteration mismatch")
    print("RTG repeat solver loop state update tests passed.")
