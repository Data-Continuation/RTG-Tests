#!/usr/bin/env python3
"""Provisional RTG quarantine export coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "quarantine-export-coherence.valid.json"

def classify(c):
    if c["claimed_export"] and not c["quarantine_released"]: return "export_overclaim_quarantined"
    if c["risk"] > 0.70: return "export_quarantine_escalated"
    if not c["quarantine_released"] and c["export_requested"]: return "export_blocked_by_quarantine"
    if c["risk"] > 0.50: return "export_release_provisional"
    return "export_after_release"
...
def main() -> None:
    ...
    print("RTG quarantine export coherence tests passed.")
