...
TMP = ROOT / "build" / "test_rtg_actual_problem_spec_yaml_generation"
...
def main():
    p = pipeline()
    text=pipeline()["spec"].read_text(); require("problem_spec_rtg_instruction_v1:" in text,"spec root missing"); require("human_or_formal_verification_required: true" in text,"verification boundary missing")
    print("RTG actual problem spec YAML generation tests passed.")
