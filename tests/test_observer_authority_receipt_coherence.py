#!/usr/bin/env python3
"""Provisional RTG observer authority receipt coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "observer-authority-receipt-coherence.valid.json"

def classify(c):
    if c["receipt_conflict"]: return "receipt_authority_quarantine"
    if not c["observer_compatible"]: return "observer_incompatible"
    if not c["authority_satisfied"]: return "authority_missing"
    if not c["receipt_present"]: return "receipt_missing"
    return "observer_authority_receipt_valid"
...
def main() -> None:
    ...
    print("RTG observer authority receipt coherence tests passed.")
