...
TMP = ROOT / "build" / "real-solver-run-request-materialization-test"
...
def main():
    ...
    out = TMP / "materialized_request.json"
    load("broker", ROOT / "scripts" / "fixture_rtg_tv_tvc_broker_receipt.py").build_broker_receipt_fixture(request, broker)
    load("materialize", ROOT / "scripts" / "materialize_rtg_real_solver_run_request.py").materialize(request, broker, out)
    payload = json.loads(out.read_text())
    require(payload["request_status"] == "materialized_for_solver_dispatch", "materialization status mismatch")
    require(payload["dispatch_inputs"]["authority_boundary"] == "tv_tvc_brokered", "authority boundary mismatch")
    require(payload["case_count"] == 2, "case count mismatch")
    print("RTG real solver-run request materialization tests passed.")
