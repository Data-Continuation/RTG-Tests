#!/usr/bin/env python3
"""Semantic-differentiation RTG dark cell color semantic behavior tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "dark-cell-color-semantic-behavior.valid.json"
VALID_STATES = {'saturated', 'dim', 'dark', 'colored'}

def classify(c):
    score = (c["evidence_density"] + c["observer_coverage"] + c["classification_stability"]) / 3
    if score >= 0.85:
        return "saturated"
    if score >= 0.60:
        return "colored"
...
def main() -> None:
    ...
    print("RTG dark cell color semantic behavior tests passed.")
