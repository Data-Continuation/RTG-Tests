#!/usr/bin/env python3
"""Provisional RTG quarantine confidence coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "quarantine-confidence-coherence.valid.json"

def classify(c):
    if c["rejection_signal"]:
        return "escalate_after_rejection"
    delta = c["new_confidence"] - c["old_confidence"]
    if c["risk_score"] <= 0.30 and c["new_confidence"] >= 0.80 and delta >= 0.15:
        return "release_after_confidence_upgrade"
    if delta <= -0.15:
        return "continue_quarantine_after_downgrade"
    if c["risk_score"] <= 0.30 and c["new_confidence"] < 0.70:
        return "provisional_release_review"
    return "retain_quarantine_stable"
...
def main() -> None:
    ...
    print("RTG quarantine confidence coherence tests passed.")
