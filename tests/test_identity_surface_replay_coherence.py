#!/usr/bin/env python3
"""Provisional RTG identity surface replay coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "identity-surface-replay-coherence.valid.json"

def classify(c):
    if c["spoof_signal"]: return "identity_spoof_quarantine"
    if c["confidence"] < 0.60: return "identity_replay_provisional"
    if not c["identity_match"] and c["replay_valid"]: return "identity_replay_mismatch"
    if c["identity_match"] and not c["replay_valid"]: return "identity_missing_replay_valid"
    return "identity_replay_valid"
...
def main() -> None:
    ...
    print("RTG identity surface replay coherence tests passed.")
