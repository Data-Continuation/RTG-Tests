...
TMP=ROOT/"build"/"theorem-candidate-registry-test"
def load(name,path):
...
def main():
    _,_,_,obl=seed_inputs(); pc=TMP/"pc.json"; out=TMP/"thm.json"
    load("pc",ROOT/"scripts"/"generate_rtg_proof_candidates.py").build_record({"proof_obligations":obl},pc)
    load("thm",ROOT/"scripts"/"register_rtg_theorem_candidates.py").build_record({"proof_candidates":pc},out)
    p=json.loads(out.read_text()); require(p["summary"]["theorem_candidate_count"]==3,"theorem count mismatch"); require(p["summary"]["review_required_count"]==0,"review mismatch")
    print("RTG theorem-candidate registry tests passed.")
