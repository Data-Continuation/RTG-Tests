...
TMP = ROOT / "build" / "coverage-geometry-scoring-test"
...
def main():
    ...
    obligations = {"schema_version":"1.0","summary":{"proof_obligation_count":3,"open_obligation_count":0,"satisfied_candidate_count":3}}
    ax = TMP/"axioms.json"; op = TMP/"operators.json"; inv = TMP/"invariants.json"; obl = TMP/"obligations.json"; out = TMP/"coverage.json"
    write_json(ax, axioms); write_json(op, operators); write_json(inv, invariants); write_json(obl, obligations)
    mod = load("coverage", ROOT/"scripts"/"score_rtg_coverage_geometry.py")
    mod.score(ax, op, inv, obl, out)
    payload = json.loads(out.read_text())
    require(payload["coverage_geometry"]["coverage_score"] >= 80, "coverage score too low")
    require(payload["coverage_geometry"]["coverage_posture"] == "formal_coverage_candidate", "coverage posture mismatch")
    print("RTG coverage geometry scoring tests passed.")
