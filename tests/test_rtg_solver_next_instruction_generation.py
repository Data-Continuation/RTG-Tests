...
TMP = ROOT / "build" / "solver-next-instruction-generation-test"
...
def main():
    p = pipeline()
    data = json.loads(p["next"].read_text())
    require(data["next_instruction_status"] == "ready_for_governed_review", "next status mismatch")
    require(len(data["next_actions"]) >= 3, "next actions missing")
    print("RTG solver next instruction generation tests passed.")
