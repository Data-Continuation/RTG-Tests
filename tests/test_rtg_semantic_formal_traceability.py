...
TMP=ROOT/"build"/"semantic-formal-traceability-test"
def load(name,path):
...
def main():
    ax,op,_,_=seed_inputs(); out=TMP/"trace.json"; load("tr",ROOT/"scripts"/"trace_rtg_semantic_formal.py").build_record({"axioms":ax,"operators":op},out)
    p=json.loads(out.read_text()); require(p["summary"]["trace_count"]==4,"trace count mismatch"); require(p["summary"]["traceability_posture"]=="traceable_candidate","posture mismatch")
    print("RTG semantic-formal traceability tests passed.")
