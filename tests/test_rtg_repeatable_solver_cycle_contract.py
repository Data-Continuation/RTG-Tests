...
TMP = ROOT / "build" / "test_rtg_repeatable_solver_cycle_contract"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["cycle"].read_text()); require(data["cycle_contract_valid"] is True,"cycle contract invalid"); require(len(data["cycle"])==6,"cycle length mismatch")
    print("RTG repeatable solver cycle contract tests passed.")
