#!/usr/bin/env python3
"""Provisional RTG governance packet replay coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "governance-packet-replay-coherence.valid.json"

def classify(c):
    expected = {"valid_replay":"accept","receipt_consistent":"accept","replay_required":"defer","invalid_replay":"reject","receipt_conflict":"quarantine"}.get(c["replay_state"])
    if c["declared_output"] != expected: return "governance_replay_output_mismatch"
    if c["replay_state"] in {"valid_replay","receipt_consistent"}: return "governance_replay_accept"
    if c["replay_state"] == "replay_required": return "governance_replay_defer"
    if c["replay_state"] == "invalid_replay": return "governance_replay_reject"
    return "governance_replay_quarantine"
...
def main() -> None:
    ...
    print("RTG governance packet replay coherence tests passed.")
