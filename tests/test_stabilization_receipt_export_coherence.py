#!/usr/bin/env python3
"""Provisional RTG stabilization receipt export coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "stabilization-receipt-export-coherence.valid.json"

VALID_STATES = ['stabilized_receipt_export_valid', 'authority_failure', 'stabilization_failure', 'receipt_failure', 'risk_quarantine']
...
def classify(case: dict[str, Any]) -> str:
    ...
    if not case["lineage"]:
        return "stabilization_failure"
    if not case["replay"]:
        return "receipt_failure"
    if not case["secondary_ok"]:
        return "receipt_failure"
    return "stabilized_receipt_export_valid"
...
def main() -> None:
    ...
    print("RTG stabilization receipt export coherence tests passed.")
