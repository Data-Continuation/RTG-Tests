...
TMP = ROOT / "build" / "solver-claim-boundary-mapping-test"
...
def main():
    p = pipeline()
    data = json.loads(p["mapping"].read_text())
    require(data["claim_boundary_status"] == "mapped_with_boundaries", "mapping status mismatch")
    require(any("Autonomous theorem proving" in x for x in data["denied_claims"]), "denied claim missing")
    print("RTG solver claim boundary mapping tests passed.")
