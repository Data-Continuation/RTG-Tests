...
TMP = ROOT / "build" / "test_rtg_actual_instruction_artifact_generation"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["instruction"].read_text()); require(data["artifact_surface"]=="rtg_actual_instruction_artifact","surface mismatch"); require(data["claim_boundary"]["solution_claim_allowed_without_external_verification"] is False,"claim boundary mismatch")
    print("RTG actual instruction artifact generation tests passed.")
