#!/usr/bin/env python3
"""Provisional RTG temporal order replay coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "temporal-order-replay-coherence.valid.json"

def classify(c):
    if c["prior_time"] > c["transition_time"] or c["transition_time"] > c["result_time"]: return "out_of_order_replay_blocked"
    if c["prior_time"] == c["transition_time"] or c["transition_time"] == c["result_time"]: return "same_time_conflict"
    if not c["replay_valid"]: return "temporal_replay_missing"
    if c["confidence"] < 0.60: return "temporal_replay_provisional"
    return "temporal_replay_valid"
...
def main() -> None:
    ...
    print("RTG temporal order replay coherence tests passed.")
