#!/usr/bin/env python3
"""Provisional RTG observer replay dark cell coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "observer-replay-dark-cell-coherence.valid.json"

def classify(c):
    if not c["replay_valid"]: return "replay_invalid_dark_quarantine"
    if not c["observer_compatible"]: return "unobserved_replay_dark"
    if c["evidence_density"] < 0.60: return "observed_replay_color_provisional"
    if c["color_intensity"] < 0.60: return "observed_replay_dark"
    return "observed_replay_colored"
...
def main() -> None:
    ...
    print("RTG observer replay dark cell coherence tests passed.")
