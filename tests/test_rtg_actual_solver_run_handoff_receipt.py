...
TMP = ROOT / "build" / "test_rtg_actual_solver_run_handoff_receipt"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["handoff"].read_text()); require(data["handoff_status"]=="ready_for_manual_or_automated_math_solver_upload","handoff mismatch"); require(data["upload_file_count"]==3,"handoff file count mismatch")
    print("RTG actual solver run handoff receipt tests passed.")
