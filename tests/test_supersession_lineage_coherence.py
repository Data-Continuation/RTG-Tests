#!/usr/bin/env python3
"""Provisional RTG supersession lineage coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "supersession-lineage-coherence.valid.json"

def classify(c):
    if c["new_version"] < c["prior_version"]:
        return "older_attempt_blocked"
    if c["new_version"] == c["prior_version"]:
        return "same_version_conflict"
    if not c["parent_present"]:
        return "missing_lineage_parent"
    if c["lineage_confidence"] < 0.60:
        return "lineage_supersession_provisional"
    return "valid_supersession_lineage"
...
def main() -> None:
    ...
    print("RTG supersession lineage coherence tests passed.")
