#!/usr/bin/env python3
"""Provisional RTG governance-output consistency tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "governance-output-consistency.valid.json"

def classify(c):
    if c["confidence"] < 0.60: return "output_provisional"
    if c["declared_output"] != c["computed_output"]: return "output_class_mismatch"
    if not c["authority_satisfied"] and c["declared_output"] == "accept": return "output_authority_mismatch"
    if not c["authority_satisfied"] and c["declared_output"] == "quarantine": return "output_receipt_missing"
    return "output_consistent"
...
def main() -> None:
    ...
    print("RTG governance-output consistency tests passed.")
