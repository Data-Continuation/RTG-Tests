...
TMP = ROOT / "build" / "test_rtg_actual_candidate_vector_seed_generation"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["vectors"].read_text()); require(len(data["candidate_vectors"])==3,"candidate vector count mismatch"); require(any(v["class"]=="false_transition_candidate" for v in data["candidate_vectors"]),"false transition vector missing")
    print("RTG actual candidate vector seed generation tests passed.")
