#!/usr/bin/env python3
"""Provisional RTG coupling-replay consistency tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "coupling-replay-consistency.valid.json"

def classify(c):
    if not c["source_replay_valid"]: return "source_replay_missing"
    if not c["target_replay_valid"]: return "target_replay_missing"
    if c["quarantine_signal"]: return "coupling_quarantine"
    if c["effect_consistency"] < 0.60: return "effect_mismatch"
    return "coupling_replay_valid"
...
def main() -> None:
    ...
    print("RTG coupling-replay consistency tests passed.")
