...
TMP = ROOT / "build" / "actual-solver-results-ingestion-bridge-test"
...
def main():
    ...
    load("bridge", ROOT / "scripts" / "bridge_rtg_actual_solver_results_ingestion.py").bridge(contract, results, out)
    payload = json.loads(out.read_text())
    require(payload["bridge_status"] == "solver_results_ready_for_rtg_ingestion", "bridge status mismatch")
    require(payload["summary"]["bridged_result_count"] == 2, "bridged result count mismatch")
    print("RTG actual solver-results ingestion bridge tests passed.")
