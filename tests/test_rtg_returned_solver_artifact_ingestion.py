...
TMP = ROOT / "build" / "test_rtg_returned_solver_artifact_ingestion"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["ingestion"].read_text()); require(data["ingestion_status"]=="returned_artifacts_ingested","ingestion status mismatch"); require(data["has_report"] is True,"report not detected")
    print("RTG returned solver artifact ingestion tests passed.")
