#!/usr/bin/env python3
"""Provisional RTG observer detector gap coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "observer-detector-gap-coherence.valid.json"

def classify(c):
    if c["claimed_coverage"] and not c["detector_available"]:
        return "coverage_overclaim"
    if not c["observer_compatible"]:
        return "identity_incompatible"
    if not c["detector_available"]:
        return "detector_gap"
    if not c["window_visible"]:
        return "window_gap"
    return "observable_covered"
...
def main() -> None:
    ...
    print("RTG observer detector gap coherence tests passed.")
