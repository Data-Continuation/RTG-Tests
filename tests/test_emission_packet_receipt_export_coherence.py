#!/usr/bin/env python3
"""Provisional RTG emission packet receipt export coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "emission-packet-receipt-export-coherence.valid.json"

def classify(c):
    if c["receipt_conflict"]: return "export_receipt_conflict"
    if c["confidence"] < 0.60: return "packet_export_provisional"
    if c["emission_allowed"] and c["export_present"] and not c["receipt_present"]: return "emission_without_receipt_blocked"
    if c["receipt_present"] and not c["export_present"]: return "receipt_without_export_deferred"
    return "packet_export_receipt_valid"
...
def main() -> None:
    ...
    print("RTG emission packet receipt export coherence tests passed.")
