#!/usr/bin/env python3
"""Provisional RTG receipt confidence regrade coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "receipt-confidence-regrade-coherence.valid.json"

def classify(c):
    if c["receipt_conflict"] or not c["receipt_consistent"]: return "receipt_conflict_reject"
    delta = c["new_confidence"] - c["old_confidence"]
    if delta >= 0.15: return "receipt_confidence_upgrade"
    if delta <= -0.15: return "receipt_confidence_downgrade"
    if c["new_confidence"] < 0.60: return "receipt_provisional"
    return "receipt_confidence_accept"
...
def main() -> None:
    ...
    print("RTG receipt confidence regrade coherence tests passed.")
