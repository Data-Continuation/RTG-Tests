...
TMP = ROOT / "build" / "test_rtg_repeat_instruction_delta"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["delta"].read_text()); require("add_next_candidate_vector_set" in data["delta_actions"],"delta action missing")
    print("RTG repeat instruction delta tests passed.")
