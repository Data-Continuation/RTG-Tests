#!/usr/bin/env python3
"""Provisional RTG emission governance output coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "emission-governance-output-coherence.valid.json"

def classify(c):
    expected = {"export_allowed":"accept","boundary_blocked":"reject","export_quarantined":"quarantine","export_deferred":"defer"}.get(c["emission_state"])
    if c["declared_output"] != expected:
        return "output_mismatch"
    if c["emission_state"] == "export_allowed": return "coherent_export_accept"
    if c["emission_state"] == "boundary_blocked": return "export_block_requires_reject"
    if c["emission_state"] == "export_quarantined": return "quarantine_requires_quarantine_output"
    return "deferred_requires_defer_output"
...
def main() -> None:
    ...
    print("RTG emission governance output coherence tests passed.")
