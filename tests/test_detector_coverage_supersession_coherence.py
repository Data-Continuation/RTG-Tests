#!/usr/bin/env python3
"""Provisional RTG detector coverage supersession coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "detector-coverage-supersession-coherence.valid.json"

def classify(c):
    if c["claimed_coverage"] and not c["detector_available"]: return "coverage_overclaim_supersession"
    if c["new_version"] < c["prior_version"]: return "older_covered_supersession_blocked"
    if not c["detector_available"]: return "detector_gap_blocks_supersession"
    if c["confidence"] < 0.60: return "covered_supersession_provisional"
    return "covered_supersession_valid"
...
def main() -> None:
    ...
    print("RTG detector coverage supersession coherence tests passed.")
