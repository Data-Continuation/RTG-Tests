#!/usr/bin/env python3
"""Provisional RTG authority supersession quarantine coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "authority-supersession-quarantine-coherence.valid.json"

def classify(c):
    if not c["authority_satisfied"]: return "authority_missing_supersession_blocked"
    if c["quarantine_risk"] > 0.75: return "supersession_quarantine_escalate"
    if c["quarantine_risk"] > 0.40: return "supersession_quarantine_continue"
    if c["confidence"] < 0.60: return "supersession_release_provisional"
    return "authorized_supersession_release"
...
def main() -> None:
    ...
    print("RTG authority supersession quarantine coherence tests passed.")
