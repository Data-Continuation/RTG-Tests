...
TMP=ROOT/"build"/"invariant-stress-test"
def load(name,path):
...
def main():
    _,_,inv,_=seed_inputs(); out=TMP/"stress.json"; load("st",ROOT/"scripts"/"stress_rtg_invariants.py").build_record({"invariants":inv},out)
    p=json.loads(out.read_text()); require(p["summary"]["stress_case_count"]==3,"stress count mismatch"); require(p["summary"]["stress_review_count"]==0,"review mismatch")
    print("RTG invariant stress test tests passed.")
