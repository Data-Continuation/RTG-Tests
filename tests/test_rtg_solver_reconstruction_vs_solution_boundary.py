...
TMP = ROOT / "build" / "solver-reconstruction-vs-solution-boundary-test"
...
def main():
    p = pipeline()
    data = json.loads(p["boundary"].read_text())
    require(data["classification"] == "candidate_not_verified_solution", "classification mismatch")
    require(data["solution_claim_allowed"] is False, "solution claim should not be allowed")
    print("RTG solver reconstruction vs solution boundary tests passed.")
