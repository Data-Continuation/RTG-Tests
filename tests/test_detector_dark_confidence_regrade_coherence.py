#!/usr/bin/env python3
"""Provisional RTG detector dark confidence regrade coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "detector-dark-confidence-regrade-coherence.valid.json"

def classify(c):
    if c["risk"] > 0.60: return "dark_confidence_quarantine"
    if not c["detector_available"]: return "detector_missing_confidence_defer"
    delta = c["new_confidence"] - c["old_confidence"]
    if delta >= 0.15: return "dark_confidence_upgraded"
    if delta <= -0.15: return "dark_confidence_downgraded"
    return "colored_confidence_stable"
...
def main() -> None:
    ...
    print("RTG detector dark confidence regrade coherence tests passed.")
