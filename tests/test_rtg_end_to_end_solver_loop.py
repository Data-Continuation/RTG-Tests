...
TMP = ROOT / "build" / "end-to-end-solver-loop-test"
...
def main():
    ...
    out = TMP / "loop.json"
    load("loop", ROOT / "scripts" / "run_rtg_end_to_end_solver_loop.py").run_loop(manifest, results, out)
    payload = json.loads(out.read_text())
    require(payload["loop_status"] == "solver_loop_ready_for_ingestion", "loop status mismatch")
    require(payload["summary"]["case_count"] == 2, "case count mismatch")
    require(payload["summary"]["result_count"] == 2, "result count mismatch")
    require("solver_result_return_sha256" in payload["receipt"], "missing return receipt")
    print("RTG end-to-end solver loop tests passed.")
