...
TMP = ROOT / "build" / "tv-tvc-authority-receipt-validation-test"
...
def main():
    ...
    receipt = TMP / "tv_tvc_authority_receipt.json"
    out = TMP / "authority_validation.json"
    ...
    load("validation", ROOT / "scripts" / "validate_rtg_tv_tvc_authority_receipt.py").validate_authority_receipt(request, receipt, out)
    payload = json.loads(out.read_text())
    require(payload["authority_receipt_valid"] is True, "authority receipt should be valid")
    require(payload["summary"]["validation_posture"] == "authority_receipt_valid", "validation posture mismatch")
    print("RTG TV/TVC authority receipt validation tests passed.")
