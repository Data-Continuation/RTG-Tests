#!/usr/bin/env python3
"""Provisional RTG classification quarantine replay loop coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "classification-quarantine-replay-loop-coherence.valid.json"

VALID_STATES = {
    "coherent",
    "authority_or_identity_failure",
    "replay_or_receipt_failure",
    "lineage_or_export_failure",
    "quarantine_required",
    "provisional_review",
    "finality_blocked",
}
...
def classify(case: dict[str, Any]) -> str:
    ...
    if not case["authority_identity"]:
        return "authority_or_identity_failure"
    if not case["replay_receipt"]:
        return "replay_or_receipt_failure"
    if not case["lineage_export"]:
        return "lineage_or_export_failure"
...
def validate(payload: dict[str, Any]) -> None:
    ...
    for case in cases:
        ...
        for field in ["authority_identity", "replay_receipt", "lineage_export", "finality_requested", "finality_allowed"]:
...
def main() -> None:
    ...
    print("RTG classification quarantine replay loop coherence tests passed.")
