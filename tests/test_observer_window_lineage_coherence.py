#!/usr/bin/env python3
"""Provisional RTG observer window lineage coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "observer-window-lineage-coherence.valid.json"

def classify(c):
    if not c["window_visible"]: return "lineage_missed_window"
    if c["lineage_conflict"]: return "lineage_observed_conflict"
    if not c["parent_present"]: return "lineage_observed_parent_missing"
    if c["confidence"] < 0.60: return "lineage_observation_provisional"
    return "lineage_observed_valid"
...
def main() -> None:
    ...
    print("RTG observer window lineage coherence tests passed.")
