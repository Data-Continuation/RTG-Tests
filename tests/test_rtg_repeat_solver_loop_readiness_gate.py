...
TMP = ROOT / "build" / "test_rtg_repeat_solver_loop_readiness_gate"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["readiness"].read_text()); require(data["repeat_loop_ready"] is True,"loop not ready"); require(data["gate_status"]=="repeatable_solver_loop_ready","gate status mismatch")
    print("RTG repeat solver loop readiness gate tests passed.")
