...
TMP = ROOT / "build" / "test_rtg_returned_solver_artifact_collection"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["collection"].read_text()); require(data["artifact_count"]>=2,"artifact collection too small"); require(data["collection_status"]=="artifacts_collected","collection status mismatch")
    print("RTG returned solver artifact collection tests passed.")
