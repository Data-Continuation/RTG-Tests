#!/usr/bin/env python3
"""Provisional RTG state-lineage integrity tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "state-lineage-integrity.valid.json"

def classify(c):
    if not c["parent_present"]: return "orphan_state"
    if c["fork_conflict"]: return "fork_conflict"
    if c["parent_mismatch"]: return "parent_mismatch"
    if c["lineage_confidence"] < 0.60: return "lineage_provisional"
...
def main() -> None:
    ...
    print("RTG state-lineage integrity tests passed.")
