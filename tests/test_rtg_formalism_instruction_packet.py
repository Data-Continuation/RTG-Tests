...
TMP = ROOT / "build" / "formalism-instruction-packet-test"
...
def main():
    p = pipeline()
    data = json.loads(p["instruction"].read_text())
    require(data["instruction_surface"] == "rtg_formalism_instruction_packet", "surface mismatch")
    require(data["target_solver_repo"] == "GCAT-BCAT-Engine/workflows", "repo mismatch")
    require(data["claim_boundary"]["autonomous_theorem_proving_claimed"] is False, "boundary mismatch")
    print("RTG formalism instruction packet tests passed.")
