#!/usr/bin/env python3
"""Provisional RTG quarantine release tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "quarantine-release.valid.json"

def classify(c):
    if c["rejection_signal"]: return "reject_release"
    if c["risk_score"] > 0.75: return "escalate_quarantine"
    if c["risk_score"] > 0.40: return "continue_quarantine"
    if c["review_confidence"] < 0.70: return "release_provisional"
    if c["remediation_verified"]: return "release_allowed"
    return "continue_quarantine"
...
def main() -> None:
    ...
    print("RTG quarantine release tests passed.")
