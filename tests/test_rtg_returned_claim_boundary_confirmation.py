...
TMP = ROOT / "build" / "test_rtg_returned_claim_boundary_confirmation"
...
def main():
    p = pipeline()
    data=json.loads(pipeline()["claim_confirm"].read_text()); require(data["claim_boundary_confirmed"] is True,"claim boundary not confirmed"); require(data["publication_claim_allowed"] is False,"publication claim should not be allowed")
    print("RTG returned claim boundary confirmation tests passed.")
