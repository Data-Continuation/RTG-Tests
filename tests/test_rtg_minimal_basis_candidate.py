...
TMP=ROOT/"build"/"minimal-basis-candidate-test"
def load(name,path):
...
def main():
    ax,op,_,_=seed_inputs(); out=TMP/"basis.json"; load("ba",ROOT/"scripts"/"select_rtg_minimal_basis.py").build_record({"axioms":ax,"operators":op},out)
    p=json.loads(out.read_text()); require(p["summary"]["minimal_basis_count"]==1,"basis count mismatch"); require(p["summary"]["selected_axiom_count"]>=1,"axiom mismatch"); require(p["summary"]["selected_operator_count"]>=1,"operator mismatch")
    print("RTG minimal-basis candidate tests passed.")
