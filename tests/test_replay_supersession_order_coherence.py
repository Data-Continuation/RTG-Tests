#!/usr/bin/env python3
"""Provisional RTG replay supersession order coherence tests."""
...
FIXTURE_PATH = ROOT / "fixtures" / "replay-supersession-order-coherence.valid.json"

def classify(c):
    if c["new_version"] < c["prior_version"]: return "older_replay_blocked"
    if c["new_version"] == c["prior_version"]: return "same_version_replay_conflict"
    if not c["replay_valid"]: return "supersession_replay_missing"
    if c["confidence"] < 0.60: return "replay_supersession_provisional"
    return "replay_supersession_valid"
...
def main() -> None:
    ...
    print("RTG replay supersession order coherence tests passed.")
