...
TMP = ROOT / "build" / "contradiction-classification-test"
...
def main():
    ...
    counterexamples = {"schema_version":"1.0","counterexamples":[{"counterexample_id":"RTG-CE-000-NONE-FOUND","counterexample_type":"none_found","status":"not_found"}],"summary":{"none_found":True}}
    ce = TMP/"counterexamples.json"; out = TMP/"contradictions.json"
    write_json(ce, counterexamples)
    mod = load("contradiction", ROOT/"scripts"/"classify_rtg_contradictions.py")
    mod.classify(ce, out)
    payload = json.loads(out.read_text())
    require(payload["summary"]["contradiction_count"] == 0, "expected zero contradictions")
    require(payload["summary"]["classification_posture"] == "no_contradiction_detected", "posture mismatch")
    print("RTG contradiction classification tests passed.")
