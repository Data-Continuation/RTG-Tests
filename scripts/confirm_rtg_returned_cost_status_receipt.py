...
import argparse
def confirm(solver_report_path: Path, output_path: Path) -> Path:
    report = load_structured(solver_report_path)
    costs = walk_numbers(report)
    status = report.get("status") or report.get("pipeline_status") or "returned_status_not_declared"
    return write_json(output_path, {
        "schema_version": "1.0",
        "generated": utc_now(),
        "confirmation_surface": "rtg_returned_cost_status_receipt_confirmation",
        "status_observed": status,
        "cost_values_observed_usd": costs,
        "max_observed_cost_usd": max(costs) if costs else None,
        "cost_status_confirmed": bool(costs),
        "receipt": {"solver_report_sha256": sha256_file(solver_report_path)}
    })
def main():
    ...
    a = p.parse_args()
    print("Wrote returned cost/status receipt confirmation: " + str(confirm(Path(a.solver_report), Path(a.output))))
